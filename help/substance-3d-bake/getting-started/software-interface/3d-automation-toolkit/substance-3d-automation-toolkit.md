---
helpx_url: "https://helpx.adobe.com/substance-3d-bake/getting-started/software-interface/substance-3d-automation-toolkit.html"
breadcrumb-title: ''
description: Automation Toolkitで使用できるベイカーのコマンドラインバージョンであるsbsbakerの使用方法について説明します。
helpx_creative_field: ""
helpx_description: bakers > Getting Started > Software Interface > Substance 3D Automation Toolkit
helpx_experience_level: ""
helpx_learn_topic: ""
helpx_tags: ""
title: Substance 3D Automation Toolkit
user-guide-description: ''
user-guide-title: ''
source-git-commit: 0197b6f5f4e3ed1f2bc0e5576bd5818d04485ed5
workflow-type: tm+mt
source-wordcount: '64'
ht-degree: 0%

---


# Substance 3D Automation Toolkit

Substance 3D Automation Toolkitには複数のツールが用意されています。そのうちの1つは&#x200B;**sbsbaker**&#x200B;で、これはSubstanceベイカーのコマンドラインバージョンです。

詳細については、[専用ドキュメント](https://helpx.adobe.com/substance-3d-sat/command-line-tools/sbsbaker.html)を参照してください。
