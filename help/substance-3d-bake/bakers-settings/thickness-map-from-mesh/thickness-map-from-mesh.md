...
breadcrumb-title: ''
description: SSSシェーダとマスキングで使用するために、メッシュサーフェスから内側にレイをキャストして厚みマップを生成します。
...
メッシュからの厚みマップはambient occlusionベイカーによく似ていますが、メッシュの表面から内部に向かって光線をキャストします。 このテクスチャは、Sub Surface Scattering(SSS)シェーダーまたはマスキングテクスチャで使用できます。
