...
| **自動トーンマッピング範囲** | 曲率値をテクスチャに書き込む方法を制御します。 有効にすると、ベイクプロセス中に見つかった最小値と最大値に基づいて、0 ～ 1の範囲の値が正規化されます。 無効になっている場合、最小値と最大値は手動で定義されます。  **注意：** UDIMやUV タイルをベイクする場合、このパラメーターを無効にしてトーンマッピングを均一にし、タイルごとに固有にしないようにしてください。無効にすると、各テクスチャ間でシームが発生する可能性があります。 適切な最小値/最大値を手動で見つけるには、まずこの設定を有効にしてベイクし、次にコンソール/ログを調べて、ベイカーが出力した値を確認します。 |
...
| **標準マップ** | 通常のテクスチャへのオプションのパス。 ベーカーの内部計算の置き換えに使用できます。 |
