...
breadcrumb-title: ''
description: '[名前によるマッチング]機能を使用すると、低ポリゴンと高ポリゴンのメッシュを切り分け、ベイク中のジオメトリのにじみを防ぐことができます。'
...
この機能は、ベイクプロセス中にジオメトリが互いににじんでテクスチャが正常に行われるのを防ぐのに非常に便利です。 同じ結果を得るために、メッシュを離す（しばしば「爆発」と呼ばれる）必要がなくなります。
