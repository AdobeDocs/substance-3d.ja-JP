...
* **共有フレームワークに貢献する一連のレイヤー**：すべてのマテリアルには基本レイヤーがあります。このレイヤーは、マテリアルの主要な色や、マテリアルが粗いか滑らかであるかなどの特性を決定します。 マテリアルには、ワニスやDustなどの効果を再現できる薄膜、コート、ファズなどのレイヤーを追加することもできます。
* **アーティスト向けの一連のコントロール**:アーティストがリフレクションフレームワークのルールや、OpenPBRマテリアル全体の外観を制御できるインターフェイスです。 これらのコントロールは、ソフトウェアのユーザーインターフェイスでの表示方法によって異なりますが、基本的にはノブまたはスライダーのセットであり、アーティストはこれを使用して、例えば、反射の強さや、特定の表示角度で表示されるカラーの濃淡を制御できます。 一部のコントロールは全体的なフレームワークに適用されます（したがって、マテリアル内のすべてのレイヤーに適用されます）。一部のコントロールは特定のレイヤーにのみ適用されます。
...
>
> * **Specular**:ベースが金属であるか否かにかかわらず、サーフェスの光沢や反射の度合いを制御します。 Specularはレイヤースタック内にありますが、それ自体が実際のレイヤーではありません。レイヤースタックに表示されるのは、ベースレイヤーとコートレイヤーのプロパティです。
> * **ジオメトリ**：他のOpenPBRレイヤーがマテリアルの構成を決定するのに対して、ジオメトリレイヤーは、マテリアルが適用される形状と外観（不透明度、法線、接線、薄壁動作など）を定義します。
>
> ここでは、ジオメトリとSpecularを引き続き「レイヤー」と呼んで、わかりやすくします。

OpenPBRサーフェスを構成する層は、最も深い層から最も外側の層まで次のとおりです。

* **ベースレイヤー**: OpenPBRマテリアルの下部にあるベースレイヤーは、光とマテリアルの間の基本的な相互作用を定義します。 このベースレイヤーのパラメーターによって、材料の主要なカラー、粗いか滑らかか、および（光との相互作用の観点から）メタリックか非メタリック（誘電体とも呼ばれます）かが決まります。

>[!NOTE]
>
> ほとんどのマテリアルには、ベースレイヤーが絶対に必要です。 この上の層（薄膜、コート、ファズ）は、3Dで再生される材料の種類に応じて存在する場合もあれば、存在しない場合もあります。
...
OpenPBRは、マテリアル作成に対する段階的なアプローチを促進します。 ほとんどのワークフローは、まずサーフェスの反応（オブジェクトから光がどのように反射するかを定義）を確立してから、透過やサブサーフェスのスキャタリングなどのボリュームエフェクトを導入します。 ファズ、エミッション、または薄膜干渉などの二次的な動作は、通常、リアリズムを調整したり、特定の視覚的な合図を実現するために、後で重ねられます。 このようにレイヤーを使用することで、アーティストは問題をより簡単に診断し、プロセスの早い段階でマテリアルが複雑になりすぎるのを防ぐことができます。 主要なビヘイビアーから二次なビヘイビアーに構築することで、マテリアルは理解、デバッグ、および再利用が容易になります。

#### 学習ツールとしてのプリセットと例

OpenPBRには一般的なマテリアルのプリセットが含まれていますが、これらは最終的な解決策ではなく、参照例として理解するのが最適です。 プリセットが粗さ、メタネス、透過深度などのパラメーターをどのようにバランスさせるかを検討すると、アーティストは特定の視覚的結果がどのように構成されているかを理解するのに役立ちます。 OpenPBRワークフローでは、プリセットの卸売に頼るのではなく、アーティストは実際のマテリアルを観察し、プレイ時の光のビヘイビアーを特定して、物理的に意味のあるコントロールを使用してそのビヘイビアーを再現できます。
...
*イエローのSpecular色を持つ誘電体（非メタリック）グレーのマテリアル。*

+++Specularパラメーター

**Specularの太さ**

「Specularカラー」では斜めの反射の色かぶりを指定し、「Specularの太さ」では0 ～ 1の範囲で反射の強さを指定します。 値を0に設定すると、グレージング角度での反射は一切発生しません。値を大きくすると、反射の強度がより顕著になります。 「実際」では、すべてのマテリアルはある程度反射しており、3Dで再作成された場合は、Specularの重み値が0より大きくなることに注意してください。 マテリアルの反射率をパラメータ化する場合、[Specularの重み]はいかなる意味でも「主要」な値と考えるべきではないことに注意してください。マテリアルの反射を決定する際には、Specularの粗さ（下記参照）が常に重要な検討事項となります。
...
このオプションでは、光が斜め（材料の表面に対してほぼ平行な角度）で反射する場合の反射に対する色かぶりを指定します。 メタリックマテリアル（下の「メタライズ」を参照）の場合は、色かぶり補正が適用されることがあります。メタリックマテリアル以外のマテリアルの場合は、Specularの色は通常、白になります。 下の図は、メタリックマテリアルと非メタリックマテリアルで異なるSpecularカラーを示しています。
...
OpenPBR素材のSpecular粗さは、PBR素材の粗さパラメーターと同様に、微細な表面変化を表します。肉眼で滑らかに見える表面でも、反射光を散乱させる小さな問題点があります。 この値は、光の反射の鋭さや広さを定義することによって、反射における表面の滑らかさや粗さを制御し、その効果を再現します。 粗さの低いマテリアルは、シャープで鏡のような反射を生み出します。 逆に、粗さの大きいマテリアルでは、ソフトでぼやけた反射が生成されます。

<table>
  <tr style="border: 0;">
    <td style="border: 0;" valign="top"><img src="../assets/openpbrf/renders/specular/roughness/roughness01.png" alt=""/><br><em>Specularの粗さ= 0.1</em></td>
    <td style="border: 0;" valign="top"><img src="../assets/openpbrf/renders/specular/roughness/roughness05.png" alt=""/><br><em>Specularの粗さ= 0.5</em></td>
    <td style="border: 0;" valign="top"><img src="../assets/openpbrf/renders/specular/roughness/roughness08.png" alt=""/><br><em>Specularの粗さ= 0.8</em></td>
  </tr>
</table>
...
IORは、マテリアルと光との相互作用の強さを表し、マテリアルに入るときに光線がどのように曲がる（屈折する）か、およびマテリアルがどの程度反射するかを、特に浅い（グレージングする）視野角度で制御します。 水や一部のプラスチックなど、反射の少ない面ではIORが低くなります。 ガラスや一部の宝石など、反射面が多いほどIORが高く、屈折効果が強くなります。 マテリアルのIORは物理的な価値であり、芸術的な解釈ではなく、客観的な数値です。 特定のマテリアルを作成する場合は、マテリアルのIORを参照し、マテリアルが光と正しく反応するように正しく設定されていることを確認するだけです。 様々なマテリアルのIORをリストする様々なソースがオンラインで利用できます。 たとえば、御影石のIORは1.43です。御影石マテリアルを作成する場合は、この値をIORとして入力します。これにより、ライトはマテリアルをリアルな方法で反射します。 IORは金属材料には影響を与えません（以下の「金属性」を参照）。 メタリックマテリアルのIOR値を変更しても、その外観には影響しません。
...
ミクロな表面変化が溝のように同じ方向にいくらか並んでいるとき、材料の反射は見る方向に依存しがちであり、溝に垂直に伸びる。 それらの溝が整列すればするほど、その効果は顕著になります。 マテリアルの異方性値によって、サーフェスの反射がすべての方向で同じように見えるか、特定の方向に伸びるかが決まります。 これにより、ブラシをかけた金属などのマテリアルの効果を再現できます。たとえば、「ブラシ効果」に沿った反射が非常に長くなります。 異方性反射は、研磨面が指紋で汚れている場合や、乾燥肌などの変形可能な面が引き伸ばされている場合にも、より微妙な方法で発生する可能性があります。
...
ある程度の異方性がある場合（つまり、マテリアルの異方性の値が0より大きい場合）、異方性タンジェントは溝の主要な方向を示します。 反射はその方向に対して垂直に伸びます。
...
OpenPBRには、マテリアルがジオメトリとどのように相互作用するか（不透明度や薄壁動作など）に影響するパラメータも含まれます。 これらのコントロールによって、サーフェスを物理的なThicknessを持つサーフェスとして扱うか、薄いシェルとして扱うかが決まります。これは、紙、葉、窓、布地などのマテリアルでは特に重要です

+++ジオメトリパラメータ

* **薄壁**：薄壁が有効になっている場合、材料は顕微鏡で薄いと見なされます。 光は、目に見える屈折がない状態でマテリアルを通過すると考えられます。
* **不透明度**:マテリアルを部分的または全体的に透過できるかどうかを決定します。 Transmissionパラメータがマテリアルの透明度を定義する一方で、Opacityパラメータはネッティングを定義するために使用できます。つまり、穴を作成するために、マテリアル情報を「除去」します。

+++

### ベースレイヤー

OpenPBRモデルの最下部にあるベースレイヤーは、光とサーフェスのマテリアル自体との基本的な相互作用を表します。 ベースレイヤーは、「ベースウェイト」、「ベースカラー」、「メタル」、「拡散の粗さ」の4つの特性で定義されます。
...
*黄色の誘電体と金属の材料を並べて配置します。*

+++ベースレイヤーの特性

* **基本重み**：基本的に、基本色（以下を参照）の強度を0から1のスケールで定義します。値0では、主に黒のマテリアル（色なし）が生成され、値1では、赤、緑、青のライトの最大量の組み合わせが生成されます。
...
* **基本色**:マテリアルの&#39;主色&#39;を決定し、メタリックベースとディフューズ（非メタリック）ベースの両方のアルベド（赤、緑、青の反射する光の量）を設定します。 前述のように、ベースカラーによって反射されるカラーが決まりますが、ベースウェイトの設定によって反射の強さが決まります。
...
* **メタル**:マテリアルが0-1のスケール（0 =誘電体、1 =完全な金属および不透明）で非金属（誘電体）または金属のどちらとして動作するかを定義します。
...
<table>
  <tr style="border: 0;">
    ...
    <td style="border: 0;" valign="top"><img src="../assets/openpbrf/renders/base/metalness/metalness1Colored.png" alt=""/><br><em>メタネス= 1.0、イエローのベースカラー</em></td>
  </tr>
</table>

* **拡散反射光の粗さ**:マテリアルの微細な表面粗さを定義します。範囲は0 （非常に滑らかで均一な反射を持つ）から1 （非常に粗い拡散反射光を持つ）で、岩や樹皮などのマテリアルに適しています。

<table>
  <tr style="border: 0;">
    <td style="border: 0;" valign="top"><img src="../assets/openpbrf/renders/base/diffuse-rough/diffuseRoughness0.png" alt=""/><br><em>拡散反射光の粗さ= 0.0</em></td>
    <td style="border: 0;" valign="top"><img src="../assets/openpbrf/renders/base/diffuse-rough/diffuseRoughness1.png" alt=""/><br><em>拡散反射光の粗さ= 1.0</em></td>
...
*サブサーフェスチャンネルを使用するマテリアル。 手やメッシュのその他の薄い領域に透光性が存在することに注目してください。*

+++サブサーフェスパラメータ

* **サブサーフェスの重み**：これは、サブサーフェスの散乱の使用量、つまり、マテリアルに入る光の量を定義します。
...
* **サブサーフェスの色**:マテリアルのサーフェスの下から再び現れる光の全体的な色を定義します。 通常、カラーを明るくすると、より目立った散乱になります。この場合、黒の値にすると、サブサーフェス散乱の効果はまったく生じません。
...
* **サブサーフェス半径**：光がマテリアル内を移動して、散乱または吸収される距離を定義します。 値を小さくすると、ライトは短距離しか移動しません。その結果、マテリアルは密集した外観になります。 半径を大きくすると、ライトはさらに遠くまで移動します。マテリアルはソフトでワックス状の半透明の外観になります。
...
* **深度**：透過カラーが最大の彩度に達するまでに光線がマテリアルを通過する距離をセンチメートル単位で定義します。つまり、透過カラーが透明な（または部分的に透明な）マテリアルを通過するときに、光がカラーを受ける速度を定義します。 透過深度の低いマテリアルの場合、光は非常に速やかに色を受け取ります。つまり、マテリアルの非常に薄い部分でも強い色で表示されます。 逆に、深度が高いと、厚い部分は非常に暗く見えたり、ほとんど不透明に見えたりし、カラー樹脂や厚い液体のような「密」な外観になります。
...
* **散乱カラー**：透明または部分的に透明なマテリアル内で散乱される光のカラーと強度を定義します。 本質的には、マテリアルの内部の「曇り」を定義し、光がマテリアル内でどのように広がり、柔らかくなるかを決定します。 散乱カラーは、光がきれいに届かないマテリアルや直線にならないマテリアル（特定のプラスチック、牛乳、濁ったアップルジュースなど）の再現に便利です。また、大きな水域（海の色合いなど）の場合でも役立ちます。
...
* **散乱の異方性**：これにより、マテリアル内でどの方向の光が散乱を起こしがちになるかが決まります。 値を0に設定すると、ライトはすべての方向に均等に散乱します。 正の値を指定すると、光は最初の光線と同じ方向に前方に散乱します。これにより、通常、マテリアルがよりクリアで、ガラスのような外観になります。 負の値を指定すると、光は光線の光源に向かって後方に散乱する傾向があります。これにより、通常、マテリアルは霜付きまたはチョークのような外観になります。
...
* **分散（アッベ）**：透明なマテリアルを通過するときに光の様々な色がどれだけ曲がるのかを定義します。これにより、光が分離したり、虹のようなフリンジが生じたり、屈折した光の中で色の付いたエッジが生じたりします。 分散(Abbe)の値を0にすると、この効果は完全に無効になります。 「分散(Abbe)」の値を小さくすると、（プリズムで見るように）色の分離がはっきりします。「分散(Abbe)」の値を大きくすると、色の分離が弱くなったり、無視できたりする程度になり、全体的により鮮明な屈折が得られます。 (分散(Abbe)パラメータは、19世紀の物理学者であり光学技術者であるErnst Abbeにちなんで名付けられました)。
...
* **Thickness**:フィルムレイヤーのThicknessをマイクロメートル単位で指定します。 物理的に正確な材質では、薄膜効果の大部分は0 ～ 1 μ mのThicknessで発生します。
...
* **屈折指数(IOR)**：前述のように、マテリアルのIORによって、マテリアルが光にどの程度強く反応するかが決まります。 OpenPBR素材の薄膜層には、独自のIORがあります。 例えば、菱形のIORは2.417です。
...
* **粗さ**：このレイヤー内の「ファズ粒子」の形状を基本的に決定します。 この値が0に近い場合、パーティクルは高く細くなります。パーティクルは、サーフェスを浅い（グレージング）角度から見ているときにより見やすくなります。 値を大きくすると、パーティクルが球状に近くなります。パーティクルはより広い角度の範囲から見やすくなります。その結果、サーフェスは全体的に粗く見えます。

<table>
  <tr>
    <td><img src="../assets/openpbrf/renders/fuzz/roughness/fuzzRoughness01.png" alt=""/><br><em>粗さ= 0.1</em></td>
    <td><img src="../assets/openpbrf/renders/fuzz/roughness/fuzzRoughness05.png" alt=""/><br><em>粗さ= 0.5</em></td>
    <td><img src="../assets/openpbrf/renders/fuzz/roughness/fuzzRoughness1.png" alt=""/><br><em>粗さ= 1.0</em></td>
  </tr>
</table>
...
このセクションでは、OpenPBRなどの最新の統合されたPBRモデルを使用して、照明条件、シーン、およびツール間で適切に動作する堅牢で予測可能なマテリアルを作成するための実用的なガイダンスに重点を置いています。 つまり、以下に示す推奨事項の多くは、PBRによるマテリアル作成に一般的に適用されます。ただし、一部の推奨事項は、OpenPBRマテリアルの特定の機能セットに依存しています。

### 実際のリファレンスから開始

物理的に基づくマテリアルは、実際の観測に基づく場合に最も信頼性が高くなります。 可能な限り、写真参照、測定値、または同様の面の直接観察に関するベースマテリアルの判断。 これは、カラーだけでなく、粗さ、反射率、サーフェスのバリエーションにも適用されます。 参照を利用すると、マテリアルをもっともな範囲内に固定できるので、再利用しやすく、照明や環境の変更に対する感受性が低くなります。 また、マテリアル内の照明の問題を補正する誘惑も減少します。

### 作成するマテリアルの物理構造のメンタルモデルを作成する

OpenPBRは、アーティストが望む外観になるまで微調整できる様々なエフェクトを可能にするパラメーターのリストではありません。 その中核は、「OpenPBR物質の層構造の概観」に記述されている基礎構造に依存しており、同様の物理層構造から構成される物質を想定している。 したがって、このモデルを念頭に置き、OpenPBRパラメータを使用してこれらのマテリアルの物理要素を記述しながらマテリアルを作成することをお勧めします。 素材の構成、顕微鏡で見た場合の縦のスライス、カラーやハイライトなど、様々な要素を考えてみましょう。 この望ましい外観を得るために必要なOpenPBRコンポーネントを予測できるように、可能な限り試してください。 同様に、別の方法を試すこともできます。つまり、一連のレイヤーからマテリアルを構築し、その究極の外観を発見することです。
...
### トランスミッション、透明度、サブサーフェススキャタリングの混乱

光を通す効果は、「透明度」または「半透明度」の下でゆるやかにグループ化されることが多いですが、OpenPBRはそれらの間に明確な違いがあります。 透過とは、ガラス、水、透明なプラスチックで見られるように、物質を通過して反対側から出る光のことです。 サブサーフェススキャタリングとは、マテリアルに入り、内部で散乱し、異なるポイントで出ていく光を表し、ソフトシャドウと内部カラーを生成します。
...
トランスミッションが適切な場合（またはその逆の場合）にサブサーフェススキャタリングを使用すると、マテリアルが過度に複雑で、レンダリングが非効率的になる可能性があります。 OpenPBRはこれらのビヘイビアーを分離するので、アーティストは参照に最も適したビヘイビアーを選択したり、必要に応じて意図的に組み合わせたりできます。

### 明確な視覚的な動機づけを持たない特徴の追加

OpenPBRは、コート層、ファズ、薄膜エフェクト、サブサーフェススキャッタリング、放射など、幅広いマテリアルのビヘイビアーを公開するため、複数の機能を一度に有効にすると便利です。 明確な参照駆動の理由なしに追加すると、材料を制御しにくくなり、視覚的にノイズが多くなります。

より信頼性の高いアプローチは、観測されたサーフェスまたはボリュームの動作に合う最も単純なマテリアルから始めて、特定の視覚的なキューが欠落している場合にのみ複雑さを追加することです。 追加の各特徴は、エッジの繊維やボリューム内の色の変化など、参照として見えるものに対応する必要があります。

### 単一の照明設定のマテリアルを作成する

物理的なワークフローは、マテリアルと照明の依存関係を減らすことを目的としていますが、特定の1つの設定でのみマテリアルが正しく見えるように調整すると、問題が発生します。 マテリアルが特定の光の強度または角度を必要とする場合、マテリアル自体を説明するのではなく、照明を補うことがよくあります。

さまざまな照明条件でマテリアルをテストすると、堅牢であるかシーンに依存しすぎるかが明らかになります。 このような柔軟性を念頭に置いて作成されたマテリアルは、様々な環境やプロジェクト間でよりスムーズに統合される傾向があります。

### 参照なしで極端なパラメータ値を使用する

OpenPBRパラメータは物理的な意味に固定されていますが、明確な意図がない極端な値に押し付けると、特に照明が変化した場合に、不安定な結果や混乱を招く可能性があります。 マテリアルが予測不可能に動作する場合、パラメータの選択を実際の参照と比較すると、問題が芸術的な意図によるものか、パラメータの誤用によるものかを判断するのに役立ちます。 リファレンスを使用して意思決定を行うことで、プロジェクト全体でマテリアルの診断、調整、および一貫性の維持が容易になります。

### モデルの限界の誤解

すべてのマテリアルをOpenPBRで表すことができるわけではありません。 他のマテリアルモデルと同様に、OpenPBRは単なるモデルです。 すでに合理的に豊富な特徴を持っていますが、存在する、または想像できる限り広大で豊かな素材の範囲に比べて、それは粗いままです。 モデルが標準で表現できるマテリアルもあれば、より多くの経験を必要とするマテリアル、モデルをその限界まで引き伸ばすマテリアル、モデルが表現できないマテリアルもあります。 場合によっては、熟練したアーティストが何らかの「不正行為」でまともな結果を得ることができます。これは通常、非物理的な選択がなされたときに行われます。 しかし、モデルで実行できる操作とできない操作を理解し、単純なマテリアルや専用のシェーダなどの代替ソリューションが必要になるタイミングを知ることが重要です。

### レンダリングの問題を解決するには、マテリアルモデルが必要です

すべての視覚的な問題がマテリアル自体から発生するわけではありません。 ノイズ、収束速度の低下、シェーディングの不具合は、OpenPBRマテリアルの定義ではなく、照明、サンプリング、レンダラーの設定が原因で発生する場合があります。

OpenPBRは物理的に一貫したマテリアルモデルを提供しますが、適切な照明およびレンダリング設定の必要性に取って代わるものではありません。 特定の変数を分離する（たとえば、簡易照明下で材料をテストする）ことは、問題が材料にあるのか他の場所にあるのかを特定するのに役立ちます。

### 最終回答ではなく、学習ツールとしてのプリセット

OpenPBRプリセットは、参照ツールおよび学習ツールとして最もよく理解されています。 プリセット値（メタル、粗さ、異方性、透過深度など）を調べると、視覚的な特定の結果がどのように構成されるかを明確にすることができます。

プリセットを最終的な解決策として使用すると、マテリアルの実際の動作が不明瞭になる可能性があります。 これらを出発点や分析例として使用することで、理解が深まり、より適応性の高い材料作成が促進されます。
...
PBR(Physically Based Rendering)は、単純なアイデアに基づいて構築されたレンダリングアプローチです。特定のライティング設定に依存するのではなく、現実世界のサーフェスの動作と一致する方法でマテリアルが光に反応する必要があります。 PBRマテリアルは、幅広い環境にわたって信頼性を維持できるように作成されているため、最新のプロダクションパイプラインでより予測可能で再利用しやすく、管理が容易になります。

この現実世界での基礎付けの直接的な結果として、PBRワークフローでは、アーティストは現実を推測するのではなく、実際の測定値に基づいて現実をコピーできます。 照明の場合、これは任意の値ではなく、物理的な単位と実世界の強度を使用することを意味します。 撮影または撮影されたコンテンツと統合するレンダリングワークフローでは、物理的にベースとなるカメラとシェーダを使用することで、実際のレンズとセンサーの視覚特性を保持できます。 マテリアルについても同じ原理により、写真測量などの手法が可能になります。この手法では、スキャンした面を手作業で作成したマテリアルとシームレスに混合できます。これは、両方とも同じ物理的前提を使用して記述されるためです。

アーティストのために、PBRはツール、エンジン、レンダラー間で共有される視覚的な言語を提供します。 PBRの原理を使用して作成されたマテリアルは、リアルタイムエンジン、パストレースレンダラー、または大きく異なる照明条件での表示でも、常に手動で調整しなくても、一貫した外観を保つことを目的としています。 この一貫性は、PBRがゲーム、VFX、ビジュアライゼーションの標準となっている主な理由です。

PBRは、光と表面に関するいくつかの基本的な物理的概念に基づいています。 ライトはサーフェスを反射、散乱、または吸収するエネルギとして扱われ、シェーダはマテリアルが不自然に明るく、または反射しないようにエネルギを節約するように設計されています。 表面の外観は、微細な粗さなどの要因によって影響を受け、反射のシャープさや柔らかさに影響を与えます。 PBRワークフローでは、これらのマテリアルタイプが基本的に異なる方法で光と相互作用するので、金属と非金属も明確に区別されます。 PBRは、物理的に派生したモデルを使用してシェーダが解釈する物理的プロパティ（ベースカラー、粗さ、メタネスなど）を記述するパラメータに依存します。

同様に重要なこととして、PBRはレンダリングプロセスのさまざまな部分の間の相互依存関係を低く促進します。 照明からマテリアルの定義を分離することで、アーティストは照明が変化するたびにマテリアルを「修正」する必要がなくなります。 この部門では、複雑な問題をより小さく管理しやすい問題に変換します。照明はマテリアルとは独立して調整でき、マテリアルは最終的なシーンの設定を知らなくても作成できます。 より細かいスケールでは、OpenPBRを含む最新のPBRモデルは、パラメーターをできる限り独立した状態に保つことを目的としており、予期しない副作用を引き起こすことなく、アーティストが値を個別に微調整できるようにします。

実際には、PBRはアーティストの役割を、照明やレンダラーの工夫を補うことから、現実世界の特性に関するマテリアルの説明にシフトします。 その結果、シーン固有の微調整よりも一貫性を優先するワークフローが実現され、手作業によるライティングのトリックではなく、明確に定義されたマテリアル入力から自然にリアリズムが生まれます。
