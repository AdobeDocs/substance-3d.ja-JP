...
OpenPBRは、この問題への対応策として設計されています。 金属、誘電体、層状物質、透過、散乱などの一般的な生産ニーズをカバーする単一の物理的に接地されたマテリアルモデルを定義することで、安定した交換目標を提供します。 これは、あらゆる状況で完全なビジュアルマッチを保証するものではありませんが、独自のシェーダーモデルと比較して、あいまいさを大幅に軽減します。

アーティストにとって現実的なテイクアウトとは、*意図*&#x200B;の維持をOpenPBRの目的とすることです。 正確な視差が不可能な場合でも、マテリアルの構造（金属、transmissive、表面の粗さ、異方性）は明確で伝達可能です。

![](../assets/OpenPBR_meetmat.jpg)

### マテリアルXとの関係

OpenPBRは、レンダラーに依存しないマテリアルや外観を表す、業界標準のフレームワークである&#x200B;**MaterialX**&#x200B;と密接に結び付けられています。 OpenPBRの参照実装はMaterialX内に存在します。つまり、OpenPBRマテリアルは、既に多くのパイプラインでサポートされている確立された交換フォーマットを使用して表現できます。

OpenPBR自体が&#x200B;**ファイル形式**&#x200B;ではないため、このリレーションシップは重要です。 代わりに、ツールの&#x200B;*内容*&#x200B;を定義し、MaterialXでは、マテリアル間でマテリアルを&#x200B;*保存して交換*&#x200B;するための標準化された手段を提供します。 実際には、これにより、マテリアルをより広範なシーン記述に埋め込み、MaterialXをサポートするDCCおよびレンダラー間で共有できます。

アーティストの場合、これは通常、内部的に発生します。しかし、現代のパイプラインではマテリアルが「ポータブル」または「相互運用性」と表現されることが増えている理由です。

### 相互運用性の意味

相互運用性に関する現実的な期待値を設定することが重要です。 OpenPBRでは、すべてのアプリケーションでマテリアルが同じように見えるとは限りません。 照明、レンダリングアルゴリズム、カラーマネジメント、機能サポートの違いは、最終的な画像にも影響を与える可能性があります。

OpenPBRによって提供される機能は共通の基準です。つまり、パラメータとビヘイビアーの一貫したセット、ツールの構築方法に関する共通の理解、マテリアルを一から再構築せずにマテリアルをツール間で転送するためのより明確な手段です。

アーティストにとっては、部署やアプリケーション間でアセットを移動する際の驚きが少なくなり、ツール固有のトリックではなく永続的なマテリアルロジックに重点を置いたワークフローになります。

### アーティストにとっての実際的な意味合い

日常の遠近法から、OpenPBRとの連携は相互運用を自然にサポートする習慣を促します。

* アプリケーション固有のマテリアルの種類ではなく、光の動作について考える
* 物理的に意味のあるパラメータの使用（メタル、ラフネス、トランスミッション、スキャッタリング）
* 文書化されていないソリューションやレンダラー固有のソリューションへの依存を回避する

マテリアルが1つのアプリを使い続ける場合でも、これらの手法は最新のパイプライン基準と一致するため、ツールやレンダラーの進化に合わせて、アセットの将来への適合性が高まります。

## マテリアルの種類

### 光との相互作用によって定義されるマテリアル

OpenPBRは、幅広い種類のマテリアルを表すことを目的としたモノリシックモデル(「uber-シェーダー」)です。このような種類は、光がそれらとどのように相互作用するかという点で説明されています。 「ガラス」や「スキン」などの固定プリセットでマテリアルを定義するのではなく、各OpenPBRマテリアルは水平および垂直方向のレイヤリングのモデルから構築されています。これにより、アーティストは拡散反射、Specular反射、透過、サブサーフェススキャタリング、レイヤリングなどの物理的に意味のある特性を完全に定義してブレンドできます。 これらのビヘイビアーのさまざまな組み合わせによって、慣れ親しんだ現実世界のマテリアルが自然に生成されます。
...
* 光がサーフェスに入り、反対側から出る
* Thicknessが外観に強く影響する
* 屈折率(IOR)によって制御され、サーフェスのラフネスによって影響を受ける屈折
* 屈折、吸収、散布、分散によって最終的な外観が決まります

透過は、光がどのように物体を通過するかを表します。 厚い部分はより暗く、彩度は高く見え、薄い部分はより鮮明に見えます。 [トランスミッションカラー]、[トランスミッション深度]、[散乱カラー]、[分散]などのパラメータは連携してこの動作を制御します。

「透明」と「transmissive」という言葉の違い：「透明」は現実の日常の言葉であり、それを通して見ることができれば何かが透明になる。 「Transmissive」は「translucency」の同義語です。 例えば、曇ったガラスは光を通すことができますが（そのため、transmissiveです）、透明ではなく見えません。

### 地下マテリアル

<table>
  <tr style="border: 0;">
    <td style="border: 0;" valign="top"><img src="../assets/openpbrf/extra/subsurface-scattering/subsurfaceScattering.png" alt=""/><br><em>表面化散乱を使用したマテリアルの例。</em></td>
    <td style="border: 0;" valign="top">サブサーフェスマテリアルを使用すると、ライトはサーフェスとその下の散乱に入り、入り口のポイントの近くで再び出ることができます。 一般的な例としては、皮膚、ワックス、大理石、様々な種類の食べ物などの多くの有機マテリアルなどがあります。 例えば、果物や野菜、サンネクタイルチーズなどです。 サブサーフェスマテリアルの特徴は次のとおりです。<br><br><br><ul><li>軟らかい拡散シェーディング</li><li>細い部分のカラーのにじみ</li><li>外観はThicknessに依存する</li><li>光は物体を透過しない</li></ul><br><br><br>表面化散乱は伝播とは異なる。 一方、透過は、マテリアルを通過して反対側から射出する光を表すのに対して、表面化散乱は、光が面に入射し、その面内で散乱し、入射したポイントの付近（ほとんどが同じ側）で射出する光を表します。 特に、マテリアルは伝播または表面化散乱をサポートしていない。 完全にメタリックしたマテリアル（つまり、ベースメタル値が1のマテリアル）のトランスミッションまたはサブサーフェス値を変更しても、その外観には影響しません。</td>
  </tr>
</table>

## マテリアルビヘイビアー間のブレンド

実際のマテリアルが完全に純粋であることはほとんどありません。 多くのサーフェスは、単一のカテゴリに属するのではなく、ビヘイビアーの組み合わせとして記述するのが最適です。 たとえば、サーフェスにDirt、摩耗、または錆の兆候がある場合、サーフェスの異なる部分が異なる方法で光に反応します。 OpenPBRは、サーフェスのある部分から別の部分へスムーズにブレンドできるようにすることで、これをサポートします。

### ブレンドとしての金属性

<table>
  <tr style="border: 0;">
    <td style="border: 0;" valign="top"><img src="../assets/openpbrf/extra/metalness/metalnessAsBlend.png" alt="" width="400"/><br><em>このマテリアルでは、鉄の金属度は1であり、錆の金属度は0である。 錆が鉄に変化するような中間のメタネス値が存在する場合があります。</em></td>
    <td style="border: 0;" valign="top">メタの値は通常0または1（完全に非メタリックまたは完全にメタリック）に設定されますが、中間値は意味を持ちます。 これらの値は、金属パーティクルやフレークを含むペイントなど、メタリックマテリアルと非メタリック物質が小さなスケールで混ざり合ったサーフェスを表します。 また、前述のように、マテリアルは個別の物理インタフェースを表すレイヤーから構築されます。 マテリアルのベース層（「コア」層）は、完全にメタリックすることが可能ですが、その上に非メタリック的なコート層を持つことも可能です。コート層は単なる追加のSpecular制御ではなく、光が通過しなければならない別個の物理的な面を表します。 例えば、自動車のペイントの種類によっては、メタリックフレークがマテリアルのベース層に表現され、コート層はクリアコートラッカーを表現します。</td>
  </tr>
</table>

### レイヤーを結合して複雑な動作を作成する

このセクションで説明したつや消しガラスや自動車ペイントなどの複雑なマテリアルは、制御された方法で複数のビヘイビアーを組み合わせて作成されます。 以下に例を示します。

* **霜付きガラス**:トランスミッションは、高ラフネスと散乱を組み合わせています
* **ペイントされた金属** :メタリックのベース上の誘電体表面で、多くの場合クリアコートを使用します。プリセットについて考えるよりも、存在する物理的なビヘイビアーと、それらが相互に作用する方法を検討する方が効果的です。 マテリアルは、光がサーフェスとどのように相互作用するかを表す、物理的に意味のあるコンポーネントによって定義されます。 マテリアルの「型」は、明示的に選択されるのではなく、ビヘイビアーの組み合わせから自然に現れます。 アーティストは、光のインタラクション、ブレンド、レイヤーに焦点を当てることで、物理的なましなさを保ちながら、様々なリアルなマテリアルを作成できます。

## OpenPBRの操作

### マテリアルの概念構造

OpenPBRは、幅広い実世界のマテリアルを表現できる単一の統合されたサーフェスシェーディングモデルとして設計されています。 マテリアルの種類ごとに異なるシェーダを切り替えるのではなく、OpenPBRは複数のサーフェス特性を組み合わせて1つのレイヤ構造にします。

概念的には、マテリアルには次の3つの重要な要素があると考えることができます。

* **基本フレームワーク**: OpenPBRでは、マテリアルが物理的な構成要素で構成されており、これらをブレンド（水平方向のミキシング）したり、重ね合わせ（垂直方向のレイヤリング）したりできます。 これらのブロックは、光に対する反応が異なる場合があります。 このようなブロックを2つブレンドすると、2つの反射がブレンドされます。 ただし、レイヤ化された場合、最下のブロックは、最上のブロックが通過できるだけの光を受け取って反射します。 この設定により、アーティストはマテリアルを単純なコンポーネントの混在と見なすことができます。 これらのコンポーネントの定義とコンポーネントが配置される場所が、2番目の重要な要素です。
...
>
> * **Specular**:ベースが金属であるか否かにかかわらず、サーフェスの光沢や反射の度合いを制御します。 Specularはレイヤースタックの中にありますが、それ自体は実際のレイヤーではなく、レイヤースタックに表示されるベースレイヤーとコートレイヤーのプロパティです。
...
* **ベースレイヤー**: OpenPBRマテリアルの下部にあるベースレイヤーは、光とマテリアルの間の基本的な相互作用を定義します。 このベースレイヤーのパラメーターによって、マテリアルの主要なカラー、粗いか滑らかか、および（光との相互作用の観点から）メタリックか非メタリック（誘電体とも呼ばれます）かが決まります。
...
ミクロ面のバラツキが溝のように同じ方向にいくぶん揃うと、マテリアルの反射は見る方向に依存しがちで、溝に垂直に伸縮します。 それらの溝が整列すればするほど、その効果は顕著になります。 マテリアルの異方性値によって、サーフェスの反射がすべての方向で同じように見えるか、特定の方向に伸びるかが決まります。 これにより、ブラシをかけた金属などのマテリアルの効果を再現できます。例えば、「ブラシ効果」に沿った反射の方がはるかに長くなります。 また、研磨面に指紋を付けたり、乾燥肌などの変形可能な面を伸縮したりすると、異方性反射が微妙に起こる場合があります。
...
* **不透明度**:マテリアルを部分的または全体的に透過できるかどうかを決定します。 Transmissionパラメータがマテリアルの透明度を定義するのに対し、Opacityパラメータはネッティングを定義するために使用できます。つまり、穴を作成するためにマテリアル情報を「取り除く」ことができます。
...
* **地表半径スケール**：平均自由曲線のカラーチャンネル依存性を制御します。 つまり、光がRGBチャンネルごとに個別にマテリアルを通過し、吸収または散乱される距離です。 これにより、地下マテリアルに特徴的なカラー変化が生じます。メッシュ内で光の届く範囲が短い場合、カラーは半径が最も長いチャンネルに向かってシフトします。\\

既定値(1, 0.5, 0.25)の場合、赤いライトが最も深い場所を通過し、その後に緑、青の順にライトが通過します。これは、スキンを含む実際の多くの地下マテリアルの動作に近いものです。
...
* **サブサーフェス異方性**:ライトがサブサーフェスマテリアル内の散乱を優先する方向を定義します。 値を0に設定すると、ライトはすべての方向に均等に散乱します。 正の値を指定すると、光は最初の光線と同じ方向に前方に散乱します。これにより、通常はマテリアルがよりクリアで半透明な外観になります。 負の値にすると、光は光線の光源に向かって後方に散乱する傾向があります。これにより、通常、マテリアルの不透明度が高くなり、密度が高くなります。
...
透過は、マテリアルを通過できる光の量を制御します。 Subsurfaceとは異なり、Transmissionはオブジェクト全体を通過する光の量を制御します。Subsurfaceは、オブジェクトの内部からサーフェスに反射される光の量を制御します。

![](../assets/openpbrf/renders/transmission/color/transmission_orange.png){width="250"}

*オレンジ色の透過色を持つtransmissiveの高いマテリアルの例。*

+++転送パラメータ

* **重量**:マテリアルの表面を通過できる光の量を制御します。 液体やガラスなどの透明なマテリアルによく使用されます。
...
物理的なマテリアルは、実際の観測結果に基づいて行うと最も信頼性が高くなります。 可能な限り、写真参照、測定値、または同様の面の直接観察に関するベースマテリアルの判断。 これは、色だけでなく、ラフネス、反射率、サーフェスのバリエーションにも適用されます。 リファレンスを使用すると、マテリアルを妥当な範囲内に配置して再利用しやすくし、照明や環境の変化に対する感度を下げることができます。 また、マテリアル内の照明の問題を補正する誘惑も減少します。

### 作成するマテリアルの物理構造のメンタルモデルがある

OpenPBRは、アーティストが望む外観になるまで微調整できる様々なエフェクトを可能にするパラメーターのリストではありません。 その中核は、「OpenPBR物質の層構造の概観」に記述されている基礎構造に依存しており、同様の物理層構造から構成される物質を想定している。 したがって、このモデルを念頭に置き、これらのマテリアルの物理的な要素をOpenPBRパラメーターで説明しながらマテリアルを作成することをお勧めします。 素材の構成、顕微鏡で見た場合の縦のスライス、カラーやハイライトなど、様々な要素を考えてみましょう。 この望ましい外観を得るために必要なOpenPBRコンポーネントを予測できるように、可能な限り試してください。 同様に、別の方法を試すこともできます。つまり、一連のレイヤーからマテリアルを構築し、その究極の外観を発見することです。
...
OpenPBRは、コート層、ファズ、薄膜エフェクト、サブサーフェススキャッタリング、放射など、幅広いマテリアルのビヘイビアーを公開するため、複数の機能を一度に有効にすると便利です。 リファレンス駆動の明確な理由なしに追加すると、マテリアルの制御が困難になり、ノイズが目立つ場合があります。
...
OpenPBRパラメータは物理的な意味に固定されていますが、明確な意図がない極端な値に押し付けると、特に照明が変化した場合に、不安定な結果や混乱を招く可能性があります。 マテリアルが予測不可能な動作をする場合、パラメーターの選択肢を実際の参照と比較すると、問題が芸術的な意図によるものか、パラメーターの誤用によるものかを判断するのに役立ちます。 リファレンスを使用して意思決定を行うことで、プロジェクト全体でマテリアルの診断、調整、および一貫性の維持が容易になります。

### モデルの限界の誤解

すべてのマテリアルがOpenPBRで表現できるわけではありません。 他のマテリアルモデルと同様に、OpenPBRは単なるモデルです。 すでに合理的に豊富な機能を持っていますが、存在する、または想像できる限り広大で緑豊かなマテリアルの範囲に比べて、それは粗いままです。 モデルが標準で表現できるマテリアルもあれば、より多くの経験を必要とするマテリアル、モデルをその限界まで引き伸ばすマテリアル、モデルが表現できないマテリアルもあります。 場合によっては、熟練したアーティストが何らかの「不正行為」でまともな結果を得ることができます。これは通常、非物理的な選択がなされたときに行われます。 しかし、モデルで何ができて、何ができないかを理解し、よりシンプルなマテリアルや専用のシェーダーなどの別のソリューションがいつ必要になるかを知ることが重要です。

### レンダリングの問題を解決するには、マテリアルモデルが必要です

すべての視覚的な問題がマテリアル自体から発生するわけではありません。 マテリアル定義ではなく、照明、サンプリング、レンダラーの設定が原因で、ノイズ、収束速度の低下、シェーディングの不具合が発生することがあります。
