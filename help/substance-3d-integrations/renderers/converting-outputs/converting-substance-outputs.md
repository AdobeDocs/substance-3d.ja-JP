...
breadcrumb-title: ''
description: Substanceのマテリアル出力を変換して、様々なレンダラー要件やワークフローに一致させる方法について説明します。
...
Substanceプラグインは、出力を生成し、特定のワークフローに合わせてマテリアルを自動的に作成します。 ただし、DCCアプリケーションおよびサードパーティレンダラーでは、メタリック/ラフ出力を手動で変換する必要がある場合があります。 次の統合は自動レンダリングワークフローをサポートしており、必要に応じて任意のマップタイプを適切に変換します。
...
カスタムSubstanceを作成している場合は、VrayやCoronaなどのレンダラーに必要な出力を作成できます。 メタリック/粗さ変換ノード（ライブラリ/PBRユーティリティ）を使用すると、ベースカラー、粗さ、メタリックマップを特定のレンダラーに簡単に変換できます。
