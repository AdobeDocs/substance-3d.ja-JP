---
helpx_url: "https://helpx.adobe.com/jp/substance-3d-integrations/game-engines/unreal-engine/unreal-engine-5/blueprints-ue5/blueprintue5-substance-material-parameters.html"
breadcrumb-title: ''
description: Unreal Engine 5では、動的なマテリアルコントロールのためのBlueprintノードを使用して、実行時にSubstanceマテリアルパラメーターを変更します。
...
helpx_learn_topic: ""
helpx_tags: ""
title: Blueprint(UE5)Substanceのマテリアルパラメーター
...
# Blueprint(UE5):Substanceマテリアルパラメータ
...
1. 「変数インスタンス」という型を参照としてSubstance グラフを作成します。\
   \**これを行うには、[My Blueprint]タブに変数を追加し、名前を付けます。 ドロップダウンで、Substance グラフインスタンス/オブジェクト参照を検索します。 変数をグラフにドラッグし、「取得」（変数名）を選択します。 [詳細]タブの[既定値]セクションでSubstance グラフインスタンスを設定します。*
1. Set Input Float Nodeを作成し、ターゲットをSubstance グラフインスタンス変数として設定します。 すべての結果を表示するには、検索ウィンドウの「コンテキスト依存」ボックスの選択を解除する必要がある場合があります。
