...
| 名前 | 入力 | 説明 |
| --- | --- | --- |
| **GetSubstances** | 入力： **マテリアル** | マテリアルによって使用されるSubstance グラフインスタンスの配列を返します。 2つの異なるグラフインスタンスのテクスチャ出力を使用するマテリアルを作成した場合、この関数はそれらの2つのグラフインスタンスを返します。 |
| **GetSubstanceTextures** | 入力： **SubstanceGraphInstance** | Substance グラフインスタンス入力パラメータから、有効なテクスチャと現在計算されているテクスチャの配列を返します。 |
...
| **DisableInstanceOutputs** | 入力： **SubstanceGraphInstance** – 無効にする出力を含むグラフインスタンス入力： **OutputIndices** （int32配列） – 無効にする出力のインデックス | 有効にすると、グラフオブジェクトに渡されたのテクスチャ出力が無効になり、削除されます |
...
| 名前 | 入力 | 説明 |
| --- | --- | --- |
| GetDynamicMaterialInstance | 入力：名前（文字列） | Substanceの実行時の動的マテリアルインスタンスを返します。インスタンスが存在しない場合は作成します。 Substance値出力からのほとんどのランタイム値変更には、動的マテリアルインスタンスが必要です。 |
