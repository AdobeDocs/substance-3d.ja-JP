...
* 新しいスクリプト作成文書
* Substance グラフエディターおよびメインサブスタンスファイルエディターでのcontrol-zのサポート
* 多数のsbsarファイルを読み込む際のパフォーマンスが向上しました。
* 複数のグラフを持つSubstanceマテリアルのサポート
* Substanceグラフを管理するためのUIを変更しました。
* C#を介してプリセットを読み込む公開API
* 更新されたオンラインドキュメント
...
* 複数入力を伴うsbsarファイルのパフォーマンスの問題
* 「ハンドルは既にリリースされています」エラー
* 「再生モードを開始すると青にシフトされる」問題
* sbsarファイルのHeightマップの問題
* 画像入力の読み取り不能テクスチャの読み取りに関する問題

既知の問題：

* 出力テクスチャのRチャンネルとBチャンネルがフリップするMacの問題を修正しました
* Macのパフォーマンスの大幅な向上
