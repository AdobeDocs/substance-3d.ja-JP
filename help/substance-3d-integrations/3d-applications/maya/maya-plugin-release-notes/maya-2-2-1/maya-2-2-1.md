---
helpx_url: "https://helpx.adobe.com/jp/substance-3d-integrations/3d-applications/maya/maya-plugin-release-notes/maya-2-2-1.html"
breadcrumb-title: ''
description: Mayaプラグインバージョン2.2.1のリリースノートを確認し、新機能、改善点、およびバグ修正について学習してください。
helpx_creative_field: ""
helpx_description: Ecosystems and Plugins > 3D Applications > Maya > Maya Plugin Release Notes > Maya 2.2.1
helpx_experience_level: ""
helpx_learn_topic: ""
helpx_tags: ""
title: Maya 2.2.1
user-guide-description: ''
user-guide-title: ''
source-git-commit: 0197b6f5f4e3ed1f2bc0e5576bd5818d04485ed5
workflow-type: tm+mt
source-wordcount: '226'
ht-degree: 0%

---


# Maya 2.2.1

Maya 2.2.1リリース：

* Substance engineを8.3.0にアップデートする
* Arnoldのネイティブサポートを追加し、ディスクへのキャッシュが不要
* これは、設定でレンダー拡張を有効にしてMayaを再起動した後に使用できます
* サポートされるバージョンは次のとおりです。
* Maya 2017 - MtoA 3.1.0/Arnold 5.2.0
* Maya 2018 - MtoA 4.0.0/Arnold 6.0.0、MtoA 4.2.0/Arnold 6.2.0
* Maya 2019 - MtoA 4.0.0/Arnold 6.0.0、MtoA 4.2.0/Arnold 6.2.0、MtoA 5.0.0/Arnold 7.0.0
* Maya 2020 - MtoA 4.0.0/Arnold 6.0.0、MtoA 4.2.0/Arnold 6.2.0、MtoA 5.0.0/Arnold 7.0.0
* Maya 2022 - MtoA 4.2.1/Arnold 6.2.0、MtoA 5.0.0/Arnold 7.0.0
* WindowsおよびMacOSの更新されたインストールディレクトリ
* macOS/Windowsのバイナリが、Adobe証明書を使用して署名されるようになりました
* sbsarの作成者がAllegorithmicまたはAllegorithmicである場合に、チャンネルの切り替えが非表示になりました
* ワークフローの複製、上書き、名前変更、削除の機能が追加された新しいワークフローUIを追加

次の新しいスクリプトコマンドが追加されました。

substancemaya

substanceGetEnableRenderingExtensions

substanceSetEnableRenderingExtensions

substanceworkflow.py

substanceWorkflowIsReadOnly

substanceWorkflowRenameWorkflow

substanceWorkflowDuplicateWorkflow

substanceWorkflowOverwriteWorkflow

substanceWorkflowRemoveWorkflow

バグ修正：

* 設定ダイアログを開く際のエラーの修正
* PYCの生成時にワークフロー機能が失敗することがなくなりました

このバージョンは、Linux、MacOS、WindowsではMaya 2017、2018、2019、2020、2022、MacOS、WindowsではMaya LT 2018、2019、2020用にリリースされています
