...
システムコンソールを使用すると、アドオンの使用中に発生したエラーを診断できます。 Blenderのシステムコンソールウィンドウが開く方法は、オペレーティングシステムによって異なります。 詳しい手順については、ブレンダーのシステムコンソール[ドキュメントページ](https://docs.blender.org/manual/en/2.79/advanced/command_line/introduction.html#console-window-status-and-error-messages)を参照してください。 コンソール出力は、テクスチャが読み込まれない、マテリアルが処理で停止するなど、予期しない問題が発生した場合に役立ちます。
...
* *アドオンをインストールする際に発生する問題。/セッション間でマテリアルの処理が停止します。 / マテリアルはセッション間でテクスチャを生成しません。 / .sbsarファイルの読み込み中にエラーが発生しました。*
...
* *サイクルレンダービューでマテリアルが更新されません*。
  * デフォルトでは、サイクルレンダービューのテクスチャは更新されません。 ただし、アドオンの環境設定で<b>サイクルのテクスチャの自動更新</b>を有効にすると、強制的に更新できます。
* サイクルのレンダービューで保存すると、パラメータが元に戻る。
  * これは、Blender側の既知のキャッシュの問題で、ビジュアル専用です。 保存時に、生成されたテクスチャファイルを更新するメッセージがリモートエンジンに送信されません。 サイクルレンダービューから切り替えて元に戻すと、テクスチャは正常に表示されます。
* *パラメーターを元に戻した後、またはパラメーターを変更した後、マテリアルが更新されなくなりました。*
  * 操作を元に戻した後、マテリアルを更新できない場合があります。 パラメーターは以前のステートに戻りますが、一致するようにテクスチャは取り消されません。 テクスチャを再び更新するには、「更新」ボタンを使用してパラメーターをデフォルトに戻し、テクスチャーを再読み込みします。
* *Substance Designerで設定された色がブレンダーのカラーピッカーでは若干異なって表示され、カラー値が同じではありません。*
  * ガンマ補正は、ブレンダーのカラーピッカーのカラーにのみ適用されます。 カラーピッカーが異なる場合がありますが、テクスチャに表示されるカラーは、Substanceアプリで設定された値に対して正確です。
* *Windowsでマテリアルを読み込むときに「wmicが認識されません」というコンソールエラーが発生しました。*
...
* *[最新の情報に更新]ボタンを使用したり、パラメーターを更新したりすると、グラフに対する変更が元に戻されます。*
  * アドオンは、変更または更新後にグラフ内の接続を更新しました。 この問題を回避するには、.sbsarから作成されたブレンダーマテリアルを複製し、新しい名前を選択します。 複製にのみノードを追加します。 テクスチャは、ノード・グループ内で更新されます。 更新するときは、これらのノードをコピーして、更新後に新しいグラフに貼り付けます。
