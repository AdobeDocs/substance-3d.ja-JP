---
helpx_url: "https://helpx.adobe.com/jp/substance-3d-integrations/3d-applications/3ds-max/3ds-max-plugin-release-notes/3ds-max-3-0-0-plus.html"
breadcrumb-title: ''
...
* スレートマテリアルエディターが開いており、Substance 2のマテリアルマップが選択されている場合、Substanceを含むIR/Productionでテクスチャ 2のレンダリングが失敗することがある問題を解決しました。
* Sampler コネクターのアップデートで、既存のノードをアップデートするのではなく、新しいSubstance 2ノードが作成される問題を解決しました。
* Substance2ノードを追加する際の3ds Maxプラグインのクラッシュの問題が解決されました。また、バッチインポートを使用して.sbsarファイルをロードしても、スクリプトエディタが開かないようになりました。
...
* 既存のすべてのアイコンをqrcファイルとrccファイルに組み込み、オートデスクの好ましい方式に合わせ、SBSAR グラフパネルで確実にロードすることにより、Substanceプラグインでのアイコン管理を標準化。
* プラグインのSubstance設定ウィンドウの応答性が向上し、ウィンドウのサイズを調整する際に入力フィールドとその説明が適切に収まるようになりました。
* SubstanceプラグインがCorona 11と互換性を持つようになりました。

<b>修正済み：</b>

* V-Ray マテリアルで光沢カラーと光沢のラフネスが自動的に接続されない問題を解決しました。 これで、V-RayとArnoldでワークフローを作成すると、両方のプロパティが自動的にリンクされます。
